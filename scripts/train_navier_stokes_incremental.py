...
# Training script on how to use the Incremental Fourier Neural Operator (FNO) to solve the Navier-Stokes equations

# Read the configuration
config_name = 'default'
pipe = ConfigPipeline([YamlConfig('./incremental.yaml', config_name='incremental', config_folder='../config'),
...
# Load the various paramaters from the config file
trainer = Trainer(model, n_epochs=config.opt.n_epochs,
                  ...
                  incremental=config.incremental.incremental_grad.use,
                  incremental_loss_gap=config.incremental.incremental_loss_gap.use, 
                  incremental_resolution=config.incremental.incremental.incremental_resolution.use, 
                  incremental_eps=config.incremental.incremental_grad.grad_explained_ratio_threshold, 
                  incremental_buffer=config.incremental.incremental_grad.buffer_modes,
                  incremental_max_iter=config.incremental.incremental_grad.max_iter, 
                  incremental_grad_max_iter=config.incremental.incremental_grad.grad_max_iter,
                  incremental_loss_eps=config.incremental.incremental_loss_gap.eps,
                  incremental_res_gap=config.incremental.incremental_resolution.epoch_gap,
                  dataset_indices=config.incremental.dataset_indices,
                  dataset_resolution=config.incremental.dataset_resolution,
                  dataset_sublist=config.incremental.dataset_sub_list)
