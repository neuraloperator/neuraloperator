...
def process_airfrans_to_fno(dataset_root, input_folder, xlen ,ylen , grid_size=(128, 128)):
    ...
    u_input = np.where(sdf_raw < 0, 0.0, v_inf*np.cos(aoa_rad))
    v_input = np.where(sdf_raw < 0, 0.0, v_inf*np.sin(aoa_rad))

    # 2. Save the SDF field separately (useful for visualization/analysis)
    sdf_fixed = np.nan_to_num(sdf_raw, nan=0.0)

    # 3. Force physics outputs (u, v, p) to 0 inside the foil
    u_target = np.nan_to_num(u_raw, nan=0.0)
    v_target = np.nan_to_num(v_raw, nan=0.0)
    p_target = np.nan_to_num(p_raw, nan=0.0)
    ...
    y_data = np.stack([u_target, v_target, p_target, nut_fixed])   # Target: flow fields
    ...
    output_file = Path(input_folder) / f"{airfoil_name}_X{xlen}_Y{ylen}_G{grid_size[0]}x{grid_size[1]}.pt"
...
def convert(dataset_root,xlen, ylen, grid_size):
    ...
    pbar1 = tqdm(sim_dirs, desc="Extracting VTPs and convert to pt")
    ...
    for sim_dir in pbar1:
        ...
        output_file = Path(sim_dir) / f"{Path(sim_dir).name}_X{xlen}_Y{ylen}_G{grid_size[0]}x{grid_size[1]}.pt"
