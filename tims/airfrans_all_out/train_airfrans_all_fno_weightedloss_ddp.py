...
from tims.airfrans_all_out.config_airfrans_all_weightedLoss_ddp import Default
import pandas as pd
import matplotlib.pyplot as plt
...
if config.wandb.log and is_logger:
    ...
    if config.wandb.log:
        ...
        run_group = config.wandb.group or wandb_name
        ...
        wandb_args = dict(
            ...
            settings=wandb.Settings(start_method="fork"),# for multi-GPU compatibility
        )
...
print(f"Config  wandb log_output: {config.wandb.log_output} and wandb_log: {config.wandb.log}")
...
# check trainer logging flags
print(f"Trainer log_output: {trainer.log_output}, wandb_log: {trainer.wandb_log}")
