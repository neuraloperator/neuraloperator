...
class AirfransOptConfig(OptimizationConfig):
    ...
    scheduler_T_max: int = 1000 # Add this line
...
class AirfransDatasetAll(ConfigBase):
    ...
    channel_dim: int = 1
    weights: List[float] = [1.0, 1.0, 1.0, 1.0]  # Weights for [u, v, Cp, nut] in loss calculation
...
class Default(ConfigBase):
    ...
    checkpoint_dir: str = "./tims/airfrans_all_out/checkpoints-all-weighted-L2-ddp"
    model: ModelConfig = FNOConfig(
        data_channels=4,    # [u_input, v_input, mask_binary, sdf_fixed] - FNO will add x,y coords automatically  
        out_channels=4,     # [u_target, v_target, p_target, nut_fixed]
        n_modes=[24, 24], 
        hidden_channels=64,
    ...
    data: AirfransDatasetAll = AirfransDatasetAll()
