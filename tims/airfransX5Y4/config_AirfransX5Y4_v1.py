...
class AirfransOptConfig(OptimizationConfig):
    n_epochs: int = 200  # check cli arguments
...
class AirfransDatasetConfig(ConfigBase):
    data_dir: str = "/home/timm/Projects/PIML/Dataset_PT_FNO/TrainingX5Y4_consolidated"
    ...
    channel_dim: int = 1
    weights: List[float] = [1.0, 1.0,1.0,1.0]  # Weights for [u, v, Cp, log_nut_ratio] in loss calculation
...
class Default(ConfigBase):
    ...
    model: ModelConfig = FNOConfig(
        ...
        out_channels=2,     # [u_target, v_target, p_target, nut_fixed]
        n_modes=[24, 24], 
        hidden_channels=64,
