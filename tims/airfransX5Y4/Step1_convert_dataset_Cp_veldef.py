...
def process_airfrans_to_pt_archive(dataset_root, input_folder,archive_dir, training_dir ,xlen, ylen, xoffset, grid_size=(128, 128)):
    ...
    if "y4" in name:
        print(f" Skipping {name} as it is a Y4 simulation  {input_folder}")
        return
    ...
    # Inlet Components
    u_inf = v_inf * np.cos(aoa_rad)
    v_inf_y = v_inf * np.sin(aoa_rad)
    nu_mol = simulation.NU
    rho = simulation.RHO
    reynolds = (v_inf * 1.0 / nu_mol)  # assuming chord=1.0
    log_re = np.log10(reynolds)
    #print(f"Processing {name}: v_inf={v_inf}, aoa={aoa_deg}, nu_mol={nu_mol}, rho={rho}, log(Re)={log_re}")
    # 2. Grid Sampling
    mesh = pv.read(Path(input_folder) / f"{name}_internal.vtu")
    ...
    x_train = np.stack([
        ...
        mask, 
        sdf_fixed,
        np.full_like(sdf_raw, log_re), # The new "Physics Scale" channel
    ])


    # 8. Package specifically for the Training Loop (Non-Dimensional Only)
    training_dict = {
...
if __name__ == "__main__":
    ...
    PATH_TO_OUTPUT = "/home/timm/Projects/PIML/Dataset_PT_FNO"
    GRID_SIZE = (64, 64)
