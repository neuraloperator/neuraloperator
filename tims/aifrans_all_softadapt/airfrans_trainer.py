...
class AirfransAllTrainer(Trainer):
    ...
    def train_one_batch(self, idx, sample, training_loss):
        ...
        #softAdapt modification
        # 2. Build the Dict SoftAdapt expects
        # Keys must match the weights dict you used in __init__
        loss_dict = {
            'u': torch.mean((out[:, 0, ...] - sample['y'][:, 0, ...])**2),
            'v': torch.mean((out[:, 1, ...] - sample['y'][:, 1, ...])**2),
            'cp': torch.mean((out[:, 2, ...] - sample['y'][:, 2, ...])**2),
            'nut': torch.mean((out[:, 3, ...] - sample['y'][:, 3, ...])**2)
        }
        # 3 update losses from this batch
        self.channel_loss_dict['u'] += loss_dict['u']
        self.channel_loss_dict['v'] += loss_dict['v']
        self.channel_loss_dict['cp'] += loss_dict['cp']
        self.channel_loss_dict['nut'] += loss_dict['nut']
