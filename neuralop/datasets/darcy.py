...
def load_darcy_241(data_path_train, data_path_test, 
                n_train, n_tests,
                batch_size, test_batch_sizes,
                test_resolutions=[241],
                train_resolution=241,
                grid_boundaries=[[0,1],[0,1]],
                positional_encoding=True,
                encode_input=False,
                encode_output=True,
    ...
    x_train = torch.tensor(data_train['coeff'], dtype=torch.float)[0:n_train, :, :].unsqueeze(channel_dim).type(torch.float32).clone()
    y_train = torch.tensor(data_train['sol'], dtype=torch.float)[0:n_train, :, :].unsqueeze(channel_dim).clone()
    ...
    x_test = torch.tensor(data_test['coeff'], dtype=torch.float)[:n_test, :, :].unsqueeze(channel_dim).type(torch.float32).clone()
    y_test = torch.tensor(data_test['sol'], dtype=torch.float)[:n_test, :, :].unsqueeze(channel_dim).clone()
