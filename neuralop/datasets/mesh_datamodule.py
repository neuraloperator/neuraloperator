...
class MeshDataModule():
    def __init__(self,
        ...
        query_res = self.range_normalize(query_res,
                                            min_b, max_b,
                                            0, 1)

        query_res = torch.from_numpy(query_res).to(torch.float32)
        constant = {'query_res': query_res}
    ...
    def compute_distances(self, mesh, query_res, signed_distance):
        mesh = o3d.t.geometry.TriangleMesh.from_legacy(mesh)
        scene = o3d.t.geometry.RaycastingScene()
        _ = scene.add_triangles(mesh)

        if signed_distance:
            dist = scene.compute_signed_distance(query_res).numpy()
        else:
            dist = scene.compute_distance(query_res).numpy()

        closest = scene.compute_closest_points(query_res)["points"].numpy()
