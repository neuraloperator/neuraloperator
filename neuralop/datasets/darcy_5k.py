...
from .transforms import GridEmbedding2D
...
def load_darcy_pt(
    ...
    return train_loader, train_db, test_loaders, data_processor

def load_darcy_421_1k(data_path, 
                        n_train, n_test,
                        batch_size, test_batch_size,
                        ...
                        channel_dim=1):
    """
    Dataloader from the PINO repo. Resolution 421x421
    Todo: some things need to be cleaned up.
    train_resolution logic is different (instead we pick the padding)
    test_resolution logic is also different
    """
    ...
    x_train = torch.tensor(a[0:n_train, ::sub, ::sub], dtype=torch.float32).unsqueeze(channel_dim)
    y_train = torch.tensor(u[0:n_train, ::sub, ::sub], dtype=torch.float32).unsqueeze(channel_dim)

    x_test = torch.tensor(a[n_train:n_train+n_test, ::sub, ::sub], dtype=torch.float32).unsqueeze(channel_dim)
    y_test = torch.tensor(u[n_train:n_train+n_test, ::sub, ::sub], dtype=torch.float32).unsqueeze(channel_dim)
    del data
    ...
    if positional_encoding:
        pos_enc = GridEmbedding2D(grid_boundaries)
    else:
        pos_enc = None
