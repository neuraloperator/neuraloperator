...
class RNO(BaseModel, name='RNO'):
    ...
    def __init__(self, n_modes,
        ...
        if self.lifting_channels:
            self.lifting = ChannelMLP(in_channels=in_channels, out_channels=self.hidden_channels, hidden_channels=self.lifting_channels, n_layers=2, n_dim=self.n_dims)
        # otherwise, make it a linear layer
        else:
            self.lifting = ChannelMLP(in_channels=in_channels, out_channels=self.hidden_channels, hidden_channels=self.hidden_channels, n_layers=1, n_dim=self.n_dims)
        ...
        if self.projection_channels:
            self.projection = ChannelMLP(in_channels=self.hidden_channels, out_channels=out_channels, hidden_channels=self.projection_channels, n_layers=2, n_dim=self.n_dims)
        # otherwise, make it a linear layer
        else:
            self.projection = ChannelMLP(in_channels=self.hidden_channels, out_channels=out_channels, hidden_channels=out_channels, n_layers=1, n_dim=self.n_dims)
