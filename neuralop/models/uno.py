...
class UNO(nn.Module):
    ...
    def __init__(self,
        ...
        assert len(
            uno_out_channels) == n_layers, "Output channels for all layers are not given"
        assert len(
            uno_n_modes) == n_layers, "number of modes for all layers are not given"
        assert len(
            uno_scalings) == n_layers, "Scaling factor for all layers are not given"
        ...
        if isinstance(self.end_to_end_scaling_factor, (float, int)):
            self.end_to_end_scaling_factor = [
                self.end_to_end_scaling_factor] * self.n_dim
        ...
        for i in range(self.n_layers):
            ...
            if i in self.horizontal_skips_map.keys():
                prev_out = prev_out + \
                    self.uno_out_channels[self.horizontal_skips_map[i]]
        ...
        for layer_idx in range(self.n_layers):
            ...
            if layer_idx in self.horizontal_skips_map.keys():
                ...
                output_scaling_factors = [
                    m / n
                    for (m, n)
                    in zip(x.shape, skip_val.shape)
                ]
                output_scaling_factors = output_scaling_factors[-1 * self.n_dim:]
                t = resample(skip_val, output_scaling_factors,
            ...
            if layer_idx in self.horizontal_skips_map.values():
                skip_outputs[layer_idx] = self.horizontal_skips[str(
                    layer_idx)](x)
