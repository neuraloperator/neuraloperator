...
class FNOGNO(nn.Module):
    def __init__(
        ...
        self.nb_search_out = NeighborSearchLayer(radius=radius).cuda()
    ...
    # x_out : (n_x, n_y, n_z, 3)
    # df : (1, n_x, n_y, n_z)
    def forward(self, x_in, x_out, df, radius=0.033):
        # x_in = x_in.cuda()
        # x_out = x_out.cuda()
        # df = df.cuda()
        # print(x_out.shape)
        ...
        x_out = self.fno.lifting(x_out.cuda())
    ...
    #     # df = df.cuda()
    #     # print(x_out.shape)
    #     out_to_in_nb = self.nb_search_out(x_out.view(-1, 3).cuda(), x_in.cuda())
    #     n_out = x_out.view(-1, 3).shape[0]
    #     x_out_embed = self.pos_embed(x_out.reshape(-1, )).reshape((n_out, -1))
    #     # Latent space and distance
    ...
    #     import pdb; pdb.set_trace()
    #     self.gno = self.gno
    #     x_out = self.gno(x_out_embed.cuda(), out_to_in_nb, x_out, x_in_embed)
    #     x_out = x_out.unsqueeze(0).permute(0, 2, 1)
    #     # Project pointwise to out channels
    #     x_out = self.projection(x_out.cuda()).squeeze(0).permute(1, 0)  # (n_in, out_channels)
    #     # import pdb; pdb.set_trace()
