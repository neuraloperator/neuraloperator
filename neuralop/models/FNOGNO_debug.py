...
from .integral_transform import IntegralTransform, NeighborSearch, NeighborSearchLayer
...
class FNOGNO(nn.Module):
    def __init__(
        ...
        self.device = "cuda" #nn.Parameter(torch.empty(0)).device
        ...
        if fno_norm == "ada_in":
            self.adain_pos_embed = PositionalEmbedding(adain_embed_dim).cuda()
        ...
        self.nb_search_out = NeighborSearchLayer().cuda()
        self.pos_embed = PositionalEmbedding(coord_embed_dim).cuda()
    ...
    def forward(self, x_in, x_out, df, radius=0.055):
        ...
        x_out = self.gno(x_out_embed.cuda(), out_to_in_nb, x=x_in_embed.cuda(), f_y=x_out.cuda(), weights=None)
    ...
    def data_dict_to_input(self, data_dict):
        ...
        if self.use_adain:
            vel = torch.tensor([data_dict['info'][0]['velocity']]).view(-1,).to(self.device)
    ...
    def example_to_input(self, example):
        x_in = example["centroids"]  # (n_in, 3)
        x_out = example["query_points"] # (n_x, n_y, n_z, 3)
        df = example["distance"].unsqueeze(0)  # (1, n_x, n_y, n_z)
        info_fields = example['inlet_velocity'] * torch.ones_like(df)

        df = torch.cat((df, info_fields), dim=0)

        if self.use_adain:
            vel = torch.tensor([example['inlet_velocity']]).view(-1, ).cuda()
            vel_embed = self.adain_pos_embed(vel)
            for norm in self.fno.fno_blocks.norm:
                norm.update_embeddding(vel_embed)
