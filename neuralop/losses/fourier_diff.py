...
class FourierDiff:
    ...
    def __init__(
        ...
        # Initialize Fourier continuation if needed
        if use_fc:
            if use_fc == "Legendre":
                self.FC_layer = FCLegendre(d=fc_degree, n_additional_pts=fc_n_additional_pts)
            elif use_fc == "Gram":
                self.FC_layer = FCGram(d=fc_degree, n_additional_pts=fc_n_additional_pts)
            else:
                raise ValueError("use_fc must be 'Legendre' or 'Gram'")
        else:
            self.FC_layer = None
    ...
    def _compute_multiple_derivatives_1d(self, u, orders):
        ...
        # Implementation from original FourierDiff1D
        if self.use_fc:
            u_extended = self.FC_layer(u, dim=1)
        else:
            u_extended = u

        # Get the length of the extended domain
        n = u_extended.shape[-1]
        dx = self.L / n

        # Compute FFT
        u_hat = torch.fft.rfft(u_extended, dim=-1)

        # Create wavenumbers using fftfreq
        k = torch.fft.rfftfreq(n, d=dx, device=u_hat.device) * (2 * torch.pi)
   
        # Apply low-pass filter if specified
        if self.low_pass_filter_ratio is not None:
            cutoff = int(u_hat.shape[-1] * self.low_pass_filter_ratio)
            u_hat[..., cutoff:] = 0
        ...
        for order in orders:
            ...
            derivative_u_hat = ((1j * k) ** order) * u_hat
            results.append(derivative_u_hat)

        derivatives_ft = torch.stack(results, dim=0)
        derivatives_real = torch.fft.irfft(derivatives_ft, dim=-1, n=n)

        # Restore original domain if Fourier continuation was used
        if self.use_fc:
            derivatives_real = self.FC_layer.restrict(derivatives_real, dim=1)
    ...
    def _compute_multiple_derivatives_2d(self, u, derivatives):
        ...
        # Implementation from original FourierDiff2D
        if self.use_fc:
            u_extended = self.FC_layer(u, dim=2)
        else:
            u_extended = u

        # Get the dimensions of the extended domain
        n_y, n_x = u_extended.shape[-2:]
        dx = self.L[1] / n_x
        dy = self.L[0] / n_y

        # FFT with transposed axes (shape -> (ny, nx))
        u_hat = torch.fft.fft2(u_extended.transpose(-2, -1), dim=(-2, -1))

        # Create wavenumbers using fftfreq
        k_x = torch.fft.fftfreq(n_x, d=dx, device=u_hat.device) * (2 * torch.pi)
        k_y = torch.fft.fftfreq(n_y, d=dy, device=u_hat.device) * (2 * torch.pi)

        # Create frequency meshgrid (note: after transpose, dimensions are (ny, nx))
        KX, KY = torch.meshgrid(k_x, k_y, indexing="ij")
   
        # Apply low-pass filter if specified
        if self.low_pass_filter_ratio is not None:
            cutoff_x = int(n_x * self.low_pass_filter_ratio)
            cutoff_y = int(n_y * self.low_pass_filter_ratio)
            u_hat[..., cutoff_y:, :] = 0
            u_hat[..., :, cutoff_x:] = 0
        ...
        for order_x, order_y in derivatives:
            ...
            KX_expanded = KX.expand(u_hat.shape)
            KY_expanded = KY.expand(u_hat.shape)

            derivative_u_hat = ((1j * KX_expanded) ** order_x) * ((1j * KY_expanded) ** order_y) * u_hat
            results.append(derivative_u_hat)
        ...
        # Restore original domain if Fourier continuation was used
        if self.use_fc:
            derivatives_real = self.FC_layer.restrict(derivatives_real, dim=2)
    ...
    def _compute_multiple_derivatives_3d(self, u, derivatives):
        ...
        # Implementation from original FourierDiff3D
        if self.use_fc:
            u_extended = self.FC_layer(u, dim=3)
        else:
            u_extended = u

        # Get the dimensions of the extended domain
        n_z, n_y, n_x = u_extended.shape[-3:]
        dx = self.L[2] / n_x
        dy = self.L[1] / n_y
        dz = self.L[0] / n_z

        # FFT with permuted axes (shape -> (nz, ny, nx))
        u_extended_permuted = u_extended.permute(*range(u_extended.ndim-3), -1, -2, -3)
        u_hat = torch.fft.fftn(u_extended_permuted, dim=(-3, -2, -1))

        # Create wavenumbers using fftfreq
        k_x = torch.fft.fftfreq(n_x, d=dx, device=u_hat.device) * (2 * torch.pi)
        k_y = torch.fft.fftfreq(n_y, d=dy, device=u_hat.device) * (2 * torch.pi)
        k_z = torch.fft.fftfreq(n_z, d=dz, device=u_hat.device) * (2 * torch.pi)

        # Create frequency meshgrid (note: after permutation, dimensions are (nz, ny, nx))
        ...
        if self.low_pass_filter_ratio is not None:
            cutoff_x = int(n_x * self.low_pass_filter_ratio)
            cutoff_y = int(n_y * self.low_pass_filter_ratio)
            cutoff_z = int(n_z * self.low_pass_filter_ratio)
            u_hat[..., :, cutoff_y:, :] = 0
            u_hat[..., :, :, cutoff_x:] = 0
            u_hat[..., cutoff_z:, :, :] = 0
        ...
        for order_x, order_y, order_z in derivatives:
            ...
            KX_expanded = KX.expand(u_hat.shape)
            KY_expanded = KY.expand(u_hat.shape)
            KZ_expanded = KZ.expand(u_hat.shape)

            derivative_u_hat = ((1j * KX_expanded) ** order_x) * ((1j * KY_expanded) ** order_y) * ((1j * KZ_expanded) ** order_z) * u_hat
            results.append(derivative_u_hat)
        ...
        # Restore original domain if Fourier continuation was used
        if self.use_fc:
            derivatives_real = self.FC_layer.restrict(derivatives_real, dim=3)
