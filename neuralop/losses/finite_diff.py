...
def get_non_uniform_fd_weights(points, num_neighbors=5, derivative_indices=[0], regularize_lstsq=False):
    """
    ...
    _, indices = torch.topk(pairwise_distances, k=k, dim=1, largest=False)
...
def non_uniform_fd(points, values, num_neighbors=5, derivative_indices=[0], regularize_lstsq=False):
    """
    ...
    indices, fd_weights = get_non_uniform_fd_weights(points=points, num_neighbors=num_neighbors, 
