...
class FiniteDiff:
    ...
    def __init__(
        ...
        # Set up grid spacing
        if dim == 1:
            self.h = h if isinstance(h, (int, float)) else h[0]
        elif dim == 2:
            if isinstance(h, (int, float)):
                ...
                self.h = h
            if len(self.h) != 2:
                raise ValueError("For 2D, h must be a float or a tuple of length 2")
        else:  # dim == 3
            if isinstance(h, (int, float)):
                self.h = (h, h, h)
            else:
                self.h = h
            if len(self.h) != 3:
                raise ValueError("For 3D, h must be a float or a tuple of length 3")
    ...
    def divergence(self, u):
        ...
        if self.dim == 1:
            if u.shape[-2] != 1:
                raise ValueError("Input must be a 1D vector field with 1 component")
    ...
    def _dx_1st_1d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dx = (torch.roll(u, -1, dims=-1) - torch.roll(u, 1, dims=-1)) / (
                2.0 * self.h
            )
    ...
    def _dx_2nd_1d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dxx = (torch.roll(u, -1, dims=-1) - 2 * u + torch.roll(u, 1, dims=-1)) / (
                self.h**2
            )
    ...
    def _dx_1st_2d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dx = (torch.roll(u, -1, dims=-2) - torch.roll(u, 1, dims=-2)) / (
                2.0 * self.h[0]
            )
    ...
    def _dy_1st_2d(self, u):
        ...
        if self.periodic_in_y:
            ...
            dy = (torch.roll(u, -1, dims=-1) - torch.roll(u, 1, dims=-1)) / (
                2.0 * self.h[1]
            )
    ...
    def _dx_2nd_2d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dxx = (torch.roll(u, -1, dims=-2) - 2 * u + torch.roll(u, 1, dims=-2)) / (
                self.h[0] ** 2
            )
    ...
    def _dy_2nd_2d(self, u):
        ...
        if self.periodic_in_y:
            ...
            dyy = (torch.roll(u, -1, dims=-1) - 2 * u + torch.roll(u, 1, dims=-1)) / (
                self.h[1] ** 2
            )
    ...
    def _dx_1st_3d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dx = (torch.roll(u, -1, dims=-3) - torch.roll(u, 1, dims=-3)) / (
                2.0 * self.h[0]
            )
    ...
    def _dy_1st_3d(self, u):
        ...
        if self.periodic_in_y:
            ...
            dy = (torch.roll(u, -1, dims=-2) - torch.roll(u, 1, dims=-2)) / (
                2.0 * self.h[1]
            )
    ...
    def _dz_1st_3d(self, u):
        ...
        if self.periodic_in_z:
            ...
            dz = (torch.roll(u, -1, dims=-1) - torch.roll(u, 1, dims=-1)) / (
                2.0 * self.h[2]
            )
    ...
    def _dx_2nd_3d(self, u):
        ...
        if self.periodic_in_x:
            ...
            dxx = (torch.roll(u, -1, dims=-3) - 2 * u + torch.roll(u, 1, dims=-3)) / (
                self.h[0] ** 2
            )
    ...
    def _dy_2nd_3d(self, u):
        ...
        if self.periodic_in_y:
            ...
            dyy = (torch.roll(u, -1, dims=-2) - 2 * u + torch.roll(u, 1, dims=-2)) / (
                self.h[1] ** 2
            )
    ...
    def _dz_2nd_3d(self, u):
        ...
        if self.periodic_in_z:
            ...
            dzz = (torch.roll(u, -1, dims=-1) - 2 * u + torch.roll(u, 1, dims=-1)) / (
                self.h[2] ** 2
            )
