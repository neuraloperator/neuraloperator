...
class Paramaters:
    def __init__(self, model, incremental, incremental_loss_gap, incremental_resolution, dataset_name) -> None:
        ...
        # Initialize ConfigPipeline to read configurations from YAML and command line arguments
        ...
        if self.incremental_resolution:
            ...
            # Best to do powers of two as FFT is faster and FFTW++ library optimizes for powers of two
            if self.dataset_name == 'SmallDarcy':
                self.sub_list = [1]  # cannot do incremental resolution
    ...
    def navier_high_sub_to_res(self, sub):
        # Calculate resolution based on sub for the NavierStokesHighFrequency dataset
        return 256 // sub
