...
@pytest.mark.parametrize("ndim", [1, 2, 3]) # Number of dimensions we want to enforce a shape on
@pytest.mark.parametrize("start_dim", [0, 1, 2]) # Dimension at which we want to start enforcing a shape
@pytest.mark.parametrize("op", ["none", "crop", "pad", "mixed"]) # Operation
def test_shape_enforcer(ndim, start_dim, op):
    # input shape
    shape = ([10] * (ndim+2))  # 10 in each dimension - represents batch, channel, and ndim spatial dimensions
    x = torch.randn(shape)
    ...
    if op == "none":
        ...
        if op == "crop":
            ...
            out[start_idx:end_idx] = [(s // 2 if i % 2 == 0 else s * 2)
