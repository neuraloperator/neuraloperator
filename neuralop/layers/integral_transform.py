...
class BatchedIntegralTransform(nn.Module):
    """Batched Integral Kernel Transform (GNO)
    Computes one of the following:
        (a) \int_{A(x)} k(x, y) dy
        (b) \int_{A(x)} k(x, y) * f(y) dy
        (c) \int_{A(x)} k(x, y, f(y)) dy
    ...
    def forward(
        ...
        assert x.ndim > 2, "Warning: BatchIntegralTransform expects\
              tensors with a batch dimension"
        ...
        self_features = torch.repeat_interleave(x, num_reps, dim=1)  
        ...
        batch_out_features = segment_csr(
            rep_features, neighbors["neighbors_row_splits"].view(1,-1), reduce=reduction
        )
